...
const MediaList = () => {
    ...
    useEffect(() => {
        ...
        fetch('https://api.themoviedb.org/3/movie/popular?language=en-US&page=1', options)
            .then(res => res.json())
            .then(res => { console.log("Popular Movies:", res.results); setPopularMovie(res.results) })
            .catch(err => console.error(err));
        fetch('https://api.themoviedb.org/3/movie/top_rated?language=en-US&page=1', options)
            .then(res => res.json())
            .then(res => { console.log("Top Trending:", res.results); setTopRated(res.results) })
            .catch(err => console.error(err));
            fetch('https://api.themoviedb.org/3/discover/movie?with_genres=27&page=1', options)
  .then(res => res.json())
  .then(res => setHorrorMovie(res.results)) // Horror movies
  .catch(err => console.error('Error fetching data:', err));

            fetch('https://api.themoviedb.org/3/tv/top_rated?page=1', options)
            .then(res => res.json())
            .then(res => { console.log("Seies:", res.results); setSeries(res.results)})
            .catch(err => console.error(err));    
    }, [])
                ...
                <div key={index} className="px-16">
                    ...
                    >
                        {section.data.map((item, i) => (
                            ...
                            <MediaDiv key={i} img={item.backdrop_path} name={item.original_title || item.original_name} id={item.id} />
                        ))}
                    </div>
                </div>
            ))}
